...
namespace logger {
  ...
  Executor::ExecutorTimer::ExecutorTimer() :
    repeated_task_id_(0), running_(false) {
  ...
  // void Executor::ExecutorTimer::PostTask_(Task task, std::chrono::microseconds& delaytime, RepeatedTaskId id, uint64_t repeat_num) {
  //   PostRepeatedTask_(std::move(task), delaytime, repeated_task_id_, repeat_num);
  // }


  RepeatedTaskId Executor::ExecutorTimer::PostRepeatedTask(Task task, const std::chrono::microseconds& delaytime, uint64_t repeat_num) {
  ...
  // 
  void Executor::ExecutorTimer::PostRepeatedTask_(Task task, const std::chrono::microseconds& delaytime, RepeatedTaskId id, uint64_t repeat_num) {
    ...
    Task func = std::bind(&Executor::ExecutorTimer::PostRepeatedTask_, this, std::move(task), delaytime, id, repeat_num - 1);
    InternalS s;
    s.time_point_ = std::chrono::high_resolution_clock::now() + delaytime;
    s.repeated_id = id;
