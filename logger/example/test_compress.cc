...
int main() {
  ...
  zip.ResetStream();
  const char* data = "This is a test string for zlib compression.This is a test string for zlib compression.This is a test string for zlib compression.";
  ...
  std::cout << "未压缩 / 压缩 = " << dataSize << "/" << out_size << " = " << static_cast<double>(dataSize) / out_size << std::endl;
