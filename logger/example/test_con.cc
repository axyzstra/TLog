...
int main() {
  ...
  // 定义一个简单的无返回值任务
  logger::Task simpleTask = []() { std::cout << "    [Task] 这是一个通过 POST_TASK 提交的简单任务。" << std::endl; };
  ...
  std::cout << "  延迟: " << std::chrono::duration_cast<std::chrono::milliseconds>(repeatDelay).count()
            << "ms, 重复次数: " << numberOfRepeats << std::endl;
